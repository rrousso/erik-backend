...
public class CommandService {
    ...
    public CommandService(StanzaRecordRepository stanzaRecordRepository) {
    ...
    private CommandResult handleSearch(String args) {
        ...
        switch (searchType) {
            case "full" -> {
                ...
                matches = stanzaRecordRepository.fullTextSearch(query);
            }
            case "setting" -> matches = stanzaRecordRepository.searchBySetting(rawKeywords);
            case "premise" -> matches = stanzaRecordRepository.searchByPremise(rawKeywords);
            case "tone" -> matches = stanzaRecordRepository.searchByTone(rawKeywords);
            case "character" -> matches = stanzaRecordRepository.searchByCharacter(rawKeywords);
            default -> {
        ...
        for (StanzaRecord stanza : matches) {
            sb.append(formatStanzaSummary(stanza));
            sb.append("\n---\n");
        }
    ...
    private CommandResult handleLoad(String idArg, SessionState state) {
        ...
        Optional<StanzaRecord> stanzaOpt = stanzaRecordRepository.findById(id);
        
        if (stanzaOpt.isEmpty()) {
            return CommandResult.handled("[System] No stanza found with ID: " + id);
        }
        
        StanzaRecord stanza = stanzaOpt.get();
    ...
    private String formatStanzaSummary(StanzaRecord stanza) {
        StringBuilder sb = new StringBuilder();
